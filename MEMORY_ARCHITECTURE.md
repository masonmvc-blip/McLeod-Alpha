...
| Cockpit settings and operator state | `control_center.py` | Cockpit/operator state | Complete: versioned settings events plus atomic JSON compatibility projections for stop alerts, parity baseline, manual-stop markers, and queued exit commands | `engine.memory.Memory` |
...
| Runtime, daily, delivery, Morning CIO, McLeod launcher, and latency-insights reports | `daily_report.py`, `execution/daily_trade_log_email.py`, `execution/opportunity_logger.py`, `control_center.py`, `cio_email/morning_report.py`, `scripts/run_mcleod_report.py`, `scripts/weekly_latency_insights.py`, `scripts/send_daily_latency_email.py`, `reports/*` | Runtime/report artifact | Complete: Memory-owned text, JSON, CSV, and append-only projections with `report-artifact.v1` events | `engine.memory.Memory` |
