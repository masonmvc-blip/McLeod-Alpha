...
def test_manifest_collects_all_pages_and_marks_transcripts_pending(tmp_path: Path) -> None:
    ...
    existing = {"recordings": [{"post_id": 2, "transcript_status": "collected", "transcript_path": "raw/2.json", "analysis_status": "complete"}]}
    manifest = build_manifest(fetch_page=fetch_page, existing=existing)
    ...
    assert manifest["recordings"][0]["transcript_status"] == "collected"
    assert manifest["recordings"][0]["transcript_path"] == "raw/2.json"
    assert manifest["recordings"][0]["analysis_status"] == "complete"
    assert manifest["recordings"][1]["transcript_status"] == "pending"
